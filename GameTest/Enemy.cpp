...
std::map<const char*, Enemy::sTextureDef > Enemy::m_textures;

float Enemy::blowUpTimer = 0.0f;

//-----------------------------------------------------------------------------
Enemy::Enemy(const char* fileName, unsigned int nColumns, unsigned int nRows, float initialMass)
    : m_nColumns(nColumns)
    , m_nRows(nRows)
    , mass(initialMass)
    , velocity(0.0f, 0.0f) // initialising velocity to zero
{
    ...
    enemyCollisionVolume = AABBVolume(Vector2(m_width / 2.0f, m_height / 2.0f));
...
void Enemy::SetFrame(unsigned int f)
{
    m_frame = f;
...
void Enemy::Shoot() {
    ...
    if (currentTime - lastShotTime >= 0.0f) { // no delay for now
        ...
        EnemyBullet newBullet(m_xpos, m_ypos, bulletDirX * bulletSpeed, bulletDirY * bulletSpeed);
...
void Enemy::UpdateBullets(float deltaTime) {
    ...
    bullets.erase(std::remove_if(bullets.begin(), bullets.end(), [](const EnemyBullet& b) {
        return b.x > 1000; // 1000 is temporary screen dimension
        }), bullets.end());
}

void Enemy::RenderBullets() {
    const float bulletLength = 15.0f;
...
bool Enemy::IsInAnimation(unsigned int animId) const {
    return m_currentAnim != -1 && m_animations[m_currentAnim].m_id == animId;
}

bool Enemy::LoadTexture(const char* filename)
{
