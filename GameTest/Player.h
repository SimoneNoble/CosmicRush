...
struct Bullet {
    ...
    Bullet(float x, float y, float speedX, float speedY) : x(x), y(y), speedX(speedX), speedY(speedY), collisionVolume(AABBVolume(Vector2(2.0f, 2.0f))) {}
};
...
class Player {
public:
    Player(const char* fileName, unsigned int nColumns, unsigned int nRows, float initialMass = 1.0f);
    ...
    AABBVolume playerCollisionVolume = AABBVolume(Vector2(playerWidth / 2.0f, playerHeight / 2.0f));
    ...
    float lastShotTime; // will make the player be able to shoot every n seconds
