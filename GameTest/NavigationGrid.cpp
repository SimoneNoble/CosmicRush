...
NavigationGrid::NavigationGrid(const std::string& filename) : NavigationGrid() {
	...
	std::string trackFileName = "RaceTrack.txt";
	//std::string filepath = "C:\\Users\\simon\\OneDrive\\Desktop\\Ubisoft\\GameTest\\TestData\\" + trackFileName;
	std::string filepath = TESTDATA_DIR + filename;
	std::ifstream infile(filepath);

	bool failedLoading = infile.fail();
	if (failedLoading) {
		//DEBUGMESSAGE(100, 200);
	}
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			infile >> type;
			n.type = type;
			n.position = Vector2((float)(x * nodeSize), (float)(y * nodeSize));
		}
	}
...
bool NavigationGrid::FindPath(const Vector2& from, const Vector2& to, NavigationPath& outPath) {
	//need to work out which node 'from' sits in, and 'to' sits in
	int fromX = ((int)from.x / nodeSize);
	int fromY = ((int)from.y / nodeSize);

	int toX = ((int)to.x / nodeSize);
	int toZ = ((int)to.y / nodeSize);

	if (fromX < 0 || fromX > gridWidth - 1 ||
		fromY < 0 || fromY > gridHeight - 1) {
	...
	GridNode* startNode = &allNodes[(fromY * gridWidth) + fromX];
