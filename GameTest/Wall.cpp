...
std::map<const char*, Wall::sTextureDef > Wall::m_textures;

float Wall::blowUpTimer = 0.0f;

//-----------------------------------------------------------------------------
Wall::Wall(const char* fileName, unsigned int nColumns, unsigned int nRows, float initialMass)
    : m_nColumns(nColumns)
    , m_nRows(nRows)
    , mass(initialMass)
    , velocity(0.0f, 0.0f) // initialising velocity to zero
{
    ...
    wallCollisionVolumeA = AABBVolume(Vector2(m_height / 2.0f, m_width / 2.0f));
    wallCollisionVolumeB = AABBVolume(Vector2(m_width / 2.0f, m_height / 2.0f));
...
void Wall::SetFrame(unsigned int f)
{
    m_frame = f;
...
bool Wall::IsInAnimation(unsigned int animId) const {
    return m_currentAnim != -1 && m_animations[m_currentAnim].m_id == animId;
}

bool Wall::LoadTexture(const char* filename)
{
